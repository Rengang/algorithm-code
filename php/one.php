...
/*
 * 二数之和
 * 给定一个整数数组和一个目标值，在数组中找到二个数的和为目标值，并返回二个数的索引。。不可重复利用数组中的数
 */
//$nums = [
//    876,
//    879,
...
{
    $res  = [];
    $temp = array_keys($nums);
    foreach ($temp as $v) {
        $diff    = $target - $nums[$v];
        $diffTmp = array_diff($temp, [$v]);
        $kes     = array_keys($nums, $diff);
        $exist   = array_values(array_intersect($diffTmp, $kes));
        if (!empty($exist)) {
            $res = [$exist[0], $v];
            break;
        }
